...
class ClockView : View {
    ...
    constructor(context: Context?, attrs: AttributeSet?, defStyleAttr: Int) : super(context,attrs,defStyleAttr){
     ...
     fun init() {
        ...
        hPaint?.strokeWidth = 7f
        //时针矩形
        hRect = RectF(-16F, 0F, 89F, 0F)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in 0..11) {
            ...
            if (i % 3 == 0) {
                ...
                paintDegree?.setStrokeWidth(5f)
                canvas.drawLine(
                    (width / 2).toFloat(),
                    (height / 2 - radius).toFloat(),
                    (width / 2).toFloat(),
                    (height / 2 - radius + 20).toFloat(),
                    paintDegree!!
                )
            } else {
        ...
        //画内层圆
        circlePaint?.let {
            canvas.drawCircle(cpoints.toFloat(), cpoints.toFloat(), (radius / 6).toFloat(),
                it
            )
        }
        //平移至中心点
